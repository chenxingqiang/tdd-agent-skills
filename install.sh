...
install_trae() {
  ...
  # 1. project_rules.md — the SINGLE file Trae actually auto-loads.
  # (Per Trae docs: rules/project_rules.md is the canonical, auto-loaded path.
  # Multiple files in rules/ are NOT auto-loaded — earlier versions of this
  # installer wrote one-file-per-skill there, which silently did nothing.)
  local rules_file="$trae_dir/rules/project_rules.md"
  mkdir -p "$(dirname "$rules_file")"
  info "Writing $rules_file (Trae auto-loads this file)"
  {
    echo "# Project Rules — tdd-agent-skills"
    echo ""
    echo "_Generated by install.sh. Re-run install to refresh._"
    echo ""
    ...
    echo "## Skill Index"
    echo ""
    echo "When the user asks for any of the tasks below, **read the matching"
    echo "skill file first** before proceeding. Skill files are full markdown"
    echo "with frontmatter; load them on demand to avoid bloating context."
    echo ""
    for skill in "${SKILL_NAMES[@]}"; do
      echo "- \`$skill\` → \`.trae/Skills/$skill/SKILL.md\`"
    done
    echo ""
    echo "## Agent Personas"
    echo ""
    echo "Invoke specialist personas by reading the matching file:"
    echo ""
    for agent_file in "$AGENTS_DIR"/*.md; do
      [ -f "$agent_file" ] || continue
      local base="$(basename "$agent_file" .md)"
      [ "$base" = "README" ] && continue
      echo "- \`$base\` → \`.trae/agents/$(basename "$agent_file")\`"
    done
  } > "$rules_file"
  success "  project_rules.md"

  # 2. Full skill bodies under .trae/Skills/ (TraeSkill community convention)
  local skills_dest="$trae_dir/Skills"
  info "Installing skills to $skills_dest/"
  mkdir -p "$skills_dest"
  for skill in "${SKILL_NAMES[@]}"; do
    local src="$SKILLS_DIR/$skill"
  ...
  # 3. Agent personas under .trae/agents/ (referenced from project_rules.md)
  local agents_dest="$trae_dir/agents"
  info "Installing agent personas to $agents_dest/"
  mkdir -p "$agents_dest"
  for agent_file in "$AGENTS_DIR"/*.md; do
    [ -f "$agent_file" ] || continue
    [ "$(basename "$agent_file" .md)" = "README" ] && continue
    cp "$agent_file" "$agents_dest/$(basename "$agent_file")"
    success "  $(basename "$agent_file")"
  done

  # 4. AGENTS.md to project root for explicit project installs.
  if $TARGET_DIR_EXPLICIT && ! $GLOBAL; then
    [ -f "$SCRIPT_DIR/AGENTS.md" ] && cp "$SCRIPT_DIR/AGENTS.md" "$TARGET_DIR/AGENTS.md" \
      && success "AGENTS.md → $TARGET_DIR/AGENTS.md"
  fi
  ...
  dim "  → Rules:  $rules_file (auto-loaded)"
  dim "  → Skills: $skills_dest/ (load on demand via path reference)"
  dim "  → Agents: $agents_dest/ (load on demand via path reference)"
  dim "  → See docs/trae-setup.md for usage tips."
}
